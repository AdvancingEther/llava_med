...
@dataclass
class LogArguments:
    wandb_project: str = field(default="default_project", metadata={"help": "The name of the wandb project"})
    wandb_entity: str = field(default=None, metadata={"help": "The entity (team) of the wandb project"})
    wandb_run_name: str = field(default="default_run", metadata={"help": "The name of the current run"})
...
def train(attn_implementation=None):
    ...
    parser = transformers.HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments, LogArguments))
    model_args, data_args, training_args, log_args = parser.parse_args_into_dataclasses()


    import wandb
    wandb.init(
        project=log_args.wandb_project,
        name=log_args.wandb_run_name
    )
    ...
    # 记录关键训练参数到wandb
    if training_args.local_rank == 0:  # 只在主进程中记录
        wandb.config.update({
            ...
            "lora_r": training_args.lora_r if training_args.lora_enable else None,
            "lora_alpha": training_args.lora_alpha if training_args.lora_enable else None,
    ...
    # 关闭wandb
    if training_args.local_rank == 0:  # 只在主进程中关闭wandb
        wandb.finish()
